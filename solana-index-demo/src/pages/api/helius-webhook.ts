...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const gotAuth = String(req.headers.authorization || '')
  if (AUTH) {
    if (gotAuth !== AUTH) {
  ...
  const events: Ev[] = Array.isArray(req.body) ? req.body : (Array.isArray(req.body?.events) ? req.body.events : [req.body])
  L({ lvl:'info', hit:true, ua: String(req.headers['user-agent']||''), len: events.length })
